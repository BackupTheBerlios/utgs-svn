...
namespace Useless {
    ...
    ImageTesselator::ImageTesselator( const PixelBuffer &pix_buf )
    {
        _flipX          = 0;
        _flipY          = 0;
        _w              = pix_buf.GetWidth();
        ...
        {   // Create MxN quad textures
            GLint wrap_s = Texture::_Wrap_s; // Record old parameter values
            GLint wrap_t = Texture::_Wrap_t;

            Texture::_Wrap_s = GL_CLAMP;     // and set u,v clamp mode
            Texture::_Wrap_t = GL_CLAMP;
            ...
            for ( y = 0, itDy = _tesselRowHeights.begin(); itDy != _tesselRowHeights.end(); ++itDy )
            {
                ...
                {
                    assert( pix_buf.BoundCheck( ptr_x, (*itDx) + 2, (*itDy) + 2 ));
                    Texture texture( (*itDx) + 2, (*itDy) + 2, row_length, fmt, ptr_x);
            ...
            Texture::_Wrap_s = wrap_s; // Load saved parameters
            Texture::_Wrap_t = wrap_t;
        }
        ...
        {   // Generate all sub-quad vertices {x,y,u,v}
            ...
            for ( y = 0, itDy = _tesselRowHeights.begin(); itDy != _tesselRowHeights.end(); ++itDy )
            {
                ...
                double x1 = 0;
                double y2 = ((double)( y + (*itDy) )) / ((double)( _h ));

                for ( x = 0, itDx = _tesselColWidths.begin(); itDx != _tesselColWidths.end(); ++itDx )
                {
                    double du = 1.0 / ((*itDx) + 1);
                    double u1 = 0.0 + du;
                    double u2 = 1.0;
                    ...
                    double x2 = ((double)( x + (*itDx) )) / ((double)( _w ));
    ...
    ImageTesselator::~ImageTesselator()
    {
    }

    // Render image as quad 1.0x1.0 at (current) origin
    void ImageTesselator::Render()
    {
    ...
    void ImageTesselator::Render( float screen_x, float screen_y, bool absolute)
    {
        glPushMatrix();
        if ( absolute)
        {
    ...
    void ImageTesselator::Render( float screen_x, float screen_y, float screen_w, float screen_h, bool absolute )
    {
        glPushMatrix();
        if ( absolute)
        {
    ...
    void ImageTesselator::SetRenderTransform( ImageTesselator::RenderTransform *T, double w_scale, double h_scale )
    {
        GAL::P4f params;
    ...
    void ImageTesselator::UseRenderTransform( ImageTesselator::RenderTransform *T, double screen_x, double screen_y )
    {
    ...
    void ImageTesselator::Clip( float min_x, float min_y, float max_x, float max_y )
    {
        GLuint x = 0, y = 0;
    ...
    void ImageTesselator::UpdateAllTextures( const PixelBuffer &pix_buf )
    {
        ...
        {   // Create MxN quad textures
            ...
            for ( y = 0, itDy = _tesselRowHeights.begin(); itDy != _tesselRowHeights.end(); ++itDy )
            {
                ...
                for ( x = 0, itDx = _tesselColWidths.begin(); itDx != _tesselColWidths.end(); ++itDx )
                {
                    int tw = (full_w > _w ? full_w - _w : (*itDx));
                    int th = (full_h > _h ? full_h - _h : (*itDy));
                    _tesselAll[ iTess++ ]._texture.LoadSubPixels( 0,0, tw+2, th+2, row_length, fmt, ptr_x);
    ...
    void ImageTesselator::UpdateAllTextures( const void *data, int width, int height, int fmt, int row_length )
    {
        assert( _w == width );
        assert( _h == height );
        ...
        {   // Create MxN quad textures
            ...
            for ( y = 0, itDy = _tesselRowHeights.begin(); itDy != _tesselRowHeights.end(); ++itDy )
            {
                ...
                for ( x = 0, itDx = _tesselColWidths.begin(); itDx != _tesselColWidths.end(); ++itDx )
                {
                    int tw = std::min( (int)(_w - full_w), (int)((*itDx) + 2) );
                    int th = std::min( (int)(_h - full_h), (int)((*itDy) + 2) );
