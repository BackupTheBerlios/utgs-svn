...
namespace Useless {
    ...
    double G_GLTesselatorStartingTexelOffset = 0.05;
    double G_GLTesselatorEndingTexelOffset = 0.05;
        ...
        {   // Create MxN quad textures
            ...
            for ( y = 0, itDy = _tesselRowHeights.begin(); itDy != _tesselRowHeights.end(); ++itDy )
            {
                ...
                for ( x = 0, itDx = _tesselColWidths.begin(); itDx != _tesselColWidths.end(); ++itDx )
                {
                    ...
                    Texture texture( *itDx, *itDy, row_length, fmt, NULL);
    ...
    void ImageTesselator::UpdateAllTextures( const PixelBuffer &pix_buf )
    {
        ...
        UpdateAllTextures( data, _w, _h, fmt, row_length );
    }
